...
export default async function QuotesPage() {
  ...
  return (
    <div className="p-8 space-y-6 max-w-7xl mx-auto">
      <div>
        <h1 className="text-2xl font-display font-bold text-[var(--text-primary)]">Quotes</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-[var(--border-color)]">
              ...
              {quotes.map((q) => (
                <tr key={q.id} className="hover:bg-[var(--bg-surface-alt)] transition-colors">
                  <td className="p-3 font-mono font-semibold text-[var(--text-primary)]">{q.reference}</td>
                  <td className="p-3 text-[var(--text-primary)]">{q.business.name}</td>
