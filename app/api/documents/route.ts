...
export async function POST(req: NextRequest) {
  ...
  const linkedQuoteRef =
    String(formData.get("linkedQuoteRef") ?? "").trim() || null;
  ...
  if (!reference) {
    return NextResponse.json(
      { error: "A document reference number is required" },
      { status: 400 },
    );
  }
  ...
  if (!business) {
    return NextResponse.json(
      { error: "Unknown business for this environment" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Could not parse that file as a spreadsheet" },
      { status: 400 },
    );
  }
